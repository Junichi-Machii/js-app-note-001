...
function getNotes() {
    let notes;
    if (localStorage.getItem("noteApp.notes") === null) notes = [];
    else notes = JSON.parse(localStorage.getItem("noteApp.notes"));
    return notes;
}
...
form.addEventListener("submit", (e)=>{
    ...
    if (titleInput.value.length > 0 && noteInput.value.length > 0) {
        ...
        titleInput.focus();
    } else showAlertMessage("題名とその内容を書いてください", "alert-message");
});
